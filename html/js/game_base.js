function ObjectPreset() {
    ...
    this.setAlpha = function(alpha, relative) {
        relative = typeof relative !== "undefined" ? relative : false;
        if (relative) this.sprite.alpha += alpha;
        else this.sprite.alpha = alpha;
        this.angle %= 1;
    };
    ...
    this.setSpriteFrame = function(imageSet, frame) {
        if (
            ...
            this.sprite.height < this.sprite.cellHeight * imageSet ||
            this.sprite.width < this.sprite.cellWidth * frame
        )
            return false;
    ...
    this.setSpriteCenter = function(centerX, centerY) {
        if (
            centerX < 0 ||
            this.sprite.cellWidth <= centerX ||
            centerY < 0 ||
            this.sprite.cellHeight <= centerY
        )
            return false;
        this.sprite.centerX = centerX;
        this.sprite.centerY = centerY;
    ...
    this.getOffsetSpritePos = function(x, y) {
        return rotationPos(
            0,
            0,
            (x - this.sprite.centerX) * this.scale.x,
            (y - this.sprite.centerY) * this.scale.y,
            this.angle
        );
    };
        ...
        if (this.visible) {
            ...
            if (view.insideSprite(this))
                drawImageEx(
                    ...
                    this.sprite.y +
                        this.sprite.cellHeight * this.sprite.imageSet,
...
function ImageManager() {
    ...
    this.loadImage = function(name, src) {
        if (name in imageList) return false;
    ...
    this.checkAllImagesLoaded = function() {
        for (var key in imageList) {
            if (!imageList[key].complete) return false;
        }
        return true;
    };
}
...
function AudioManager() {
    ...
    this.loadAudio = function(name, src) {
        if (name in audioList) return false;
    ...
    this.setAudioVolume = function(name, volume) {
        ...
        if (name in audioList) audioList[name].volume = volume;
        else return false;
        return true;
    };
    ...
    this.checkAllAudiosLoaded = function() {
        for (var key in audioList) {
            if (!audioList[key].complete) return false;
        }
        return true;
    };
}
...
function ObjectManager(gameObject) {
    ...
    this.addObjectNow = function(object) {
        if (objectList.indexOf(object) != -1) return false;
...
function RoomManager(gameObject) {
    ...
    this.addRoom = function(name, room) {
        if (name in roomList) return false;
...
function InputManager(gameObject, _target) {
    _target = typeof _target !== "undefined" ? _target : window;
    this.gameObject = gameObject;
    this.keyStates = {};
    this.mouseStates = {};
    this.canvasMousePos = {
    ...
    var contextViewRatioX = 1;
    var contextViewRatioY = 1;
    ...
    this.getMouseState = function(mouse) {
        return this.mouseStates.hasOwnProperty(mouse)
            ? this.mouseStates[mouse]
            : 0;
    };
    ...
    window.addEventListener("mouseout", function(e) {
        if (e.toElement === null) asynMouseStates = {};
    });
}
...
function View(width, height) {
    ...
    this.setZoomSpeed = function(speed) {
        if (0 <= speed) this.zoomSpeed = speed;
    };
        ...
        if (this.zoom != this.targetZoom) {
            if (this.zoomSpeed == 0) {
                ...
                this.zoom = lerp(
                    this.zoom,
                    this.targetZoom,
                    this.zoomSpeed * tickTimeMul,
                    true
                );
                if (
                    diff(
                        this.baseWidth * this.zoom,
                        this.baseWidth * this.targetZoom
                    ) < 1
                )
                    this.zoom = this.targetZoom;
            }
...
function FPSCounter(_maxQueued) {
    var maxQueued = _maxQueued;
    var fpsQueue = [];
    var averageFPS = 0;
    var show = true;
    ...
    this.draw = function(context) {
        if (show) {
            ...
            context.fillText(
                averageFPS.toFixed().toString() + "fps",
    ...
    this.addFPS = function(fps) {
        if (fps === Infinity) fps = 999;
        fpsQueue.push(fps);
        if (maxQueued < fpsQueue.length) fpsQueue.shift();
    };
...
function Game(canvas, targetFPS, runGame) {
    ...
    this.checkAllAssetsLoaded = function() {
        if (
            this.image_m.checkAllImagesLoaded() &&
            this.audio_m.checkAllAudiosLoaded()
        )
            this.init();
        else
            setTimeout(function() {
                _this.checkAllAssetsLoaded();
            }, 100);
    };
    ...
    this.run = function() {
        if (ready && this.loopCallback == null) {
            ...
            this.loopCallback = setTimeout(function() {
                _this.loop();
            }, 0);
        }
    };
    ...
    this.loop = function() {
        ...
        if (targetFrameTime <= deltaTime) {
            ...
            if (targetFrameTime < deltaTime) deltaTime = targetFrameTime;
        }
    ...
    this.objectUpdate = function(obj, func) {
        ...
        if (obj.constructor === Array) {
            ...
            while (i < len) {
                if (func == -1) obj[i].preUpdate();
                else if (func == 1) obj[i].postUpdate();
                else obj[i].update();
            ...
            if (func == -1) obj.preUpdate();
            else if (func == 1) obj.postUpdate();
            else obj.update();
    ...
    this.objectDraw = function(obj, context, func) {
        ...
        if (obj.constructor === Array) {
            ...
            while (i < len) {
                if (func == -1) obj[i].preDraw(context);
                else if (func == 1) obj[i].postDraw(context);
                else obj[i].draw(context);
            ...
            if (func == -1) obj.preDraw(context);
            else if (func == 1) obj.postDraw(context);
    ...
    this.objectUpdateAll = function(object_m, pre, main, post) {
        ...
        var len,
            layersLen = depthLayers.length,
            loopSetLen = loopSet.length;
        var depthLayer, objectList;
    ...
    this.performProcesss = function() {
        ...
        if (this.room != null) this.room.object_m.performRemoveObjects();

        this.object_m.performAddObjects();
        if (this.room != null) this.room.object_m.performAddObjects();
...
function CollisionMesh(type, args) {
    ...
    if (type == "Point") {
        ...
        if (7 <= arguments.length && arguments.length % 2 == 1) return;
        this.points = [];
        var i = 1,
